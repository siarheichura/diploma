...
export function StickerBasket() {
  ...
  const sendMessage = () => {
    ...
    basket.forEach((item: any) => {
      let message = `<b>icon:</b> ${item.icon}%0A<b>nickname:</b> ${item.nickname}%0A<b>carBrand:</b> ${item.carBrand}%0A<b>phoneNum:</b> ${item.phoneNum}%0A<b>adText:</b> ${item.adText}%0A<b>width:</b> ${item.width}%0A<b>height:</b> ${item.height}%0A<b>color:</b> ${item.color}%0A<b>comment:</b> ${item.comment}%0A<b>count:</b> ${item.count}%0A<b>price:</b> ${item.price}`;
        ...
        {basket.map((sticker: any) => {
          return (
            <div className={styles.basketItem}>
              ...
              <div className={styles.stickerCount}>{sticker.count} pieces</div>
